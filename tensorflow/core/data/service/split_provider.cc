...
Status DataServiceSplitProvider::Save(
  ...
  VLOG(0) << "data service split saving, s1:" << s1 << ", s2:" << s2
          << " name: " << full_name(kRepetition) << " val:" << repetition_;
  if (skip_all_) {
...
Status DataServiceSplitProvider::Restore(
  ...
  if (initialized_) {
    VLOG(0) << "Refusing to restore twice: " << full_name("")
            << ". first restore is what matters...";
    return Status::OK();
  }
  initialized_ = true;
  VLOG(0) << "Checking key " << full_name(kRepetition)
          << " local state: " << target_index_ << ", " << repetition_;
  if (!reader->Contains(full_name(kRepetition)) ||
      reader->Contains(full_name(kSkipAll))) {
  ...
  TF_RETURN_IF_ERROR(
      reader->ReadScalar(full_name(kTargetIndex), &target_index_));
