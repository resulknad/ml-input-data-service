...
void DispatcherState::RegisterWorker(
  ...
  jobs_by_worker_[address] = absl::flat_hash_map<int64, std::shared_ptr<Job>>();
}
...
void DispatcherState::CreatePendingTask(
  ...
  task =
      std::make_shared<Task>(task_id, job, create_pending_task.worker_address(),
                             ...
                             create_pending_task.dataset_key());
...
void DispatcherState::ClientHeartbeat(
  ...
  if (client_heartbeat.has_task_rejected()) {
    ...
    VLOG(0) << "DBK (ClientHeartbeat): task failures " << task.failures
            << " for task " << task.task->task_id << " on worker "
            << task.task->worker_address;
