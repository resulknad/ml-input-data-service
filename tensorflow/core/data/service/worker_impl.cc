...
Status DataServiceWorkerImpl::Heartbeat() TF_LOCKS_EXCLUDED(mu_) {
  ...
  *request.mutable_current_tasks() = {current_tasks.begin(),
                                      current_tasks.end()};
  *request.mutable_new_tasks() = {new_tasks.begin(),
                                  new_tasks.end()};
  *request.mutable_task_ids_to_delete() = {task_ids_to_delete.begin(),
                                           task_ids_to_delete.end()};
  request.set_tasks_metrics(tasks_metrics);
