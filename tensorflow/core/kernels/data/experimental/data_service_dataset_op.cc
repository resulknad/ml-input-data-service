  ...
  class Iterator : public DatasetIterator<Dataset> {
   public:
    ...
    Status GetNextInternal(IteratorContext* ctx,
      ...
      } while (result.skip || (result.end_of_sequence && (!Finished() || results_.size() > 1))); // EASL - handle element reordering.
      ...
      while (true) {
        Result* result;
        {
          ...
          if (task_to_process) {
            ...
            task_to_process->num_outstanding_requests--; // EASL
...
void DataServiceDatasetOp::MakeDataset(OpKernelContext* ctx,
  ...
  *output = new Dataset(
      ...
      task_refresh_interval_hint_ms_, target_workers_, iteration_counter,
