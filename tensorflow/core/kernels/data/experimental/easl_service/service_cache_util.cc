...
std::string GetFileName(const std::string& shard_directory, int64_t task_id, uint64 writer_id, 
                        uint64 file_id, std::string prefix_hash) {
  return io::JoinPath(shard_directory, strings::Printf("%d_%s_%02llu_%08llu.easl",
                      task_id,
                      prefix_hash.c_str(), 
...
Writer::Writer(Env* env,
    ...
    writer_count_(writer_count), writer_version_(writer_version),
...
void MultiThreadedAsyncWriter::Initialize(Env *env, int64 file_index, const std::string &shard_directory,
        ...
        const DataTypeVector &output_types, std::function<void (Status)> done, int64_t task_id) {
  ...
  for (int i = 0; i < writer_count_; ++i) {
    thread_pool_->Schedule(
            [this, env, shard_directory, task_id, checkpoint_id, compression, version,
...
void MultiThreadedAsyncWriter::Write(const std::vector<Tensor>& tensors) {
  ...
  VLOG(0) << "before enquueing to writer: " << element.value.back().SummarizeValue(100);
...
Status MultiThreadedAsyncWriter::WriterThread(Env* env, 
  ...
  TF_RETURN_IF_ERROR(snapshot_util::Writer::Create(
          env, GetFileName(shard_directory, task_id, writer_id, file_index, 
  ...
  while (true) {
    ...
    VLOG(0) << "Writer " << writer_id << " wrote el_id " << be.value.back().SummarizeValue(100);
...
void MultiThreadedAsyncReader::Consume(string* s, bool* end_of_sequence) {
  ...
  if(!status.ok()){
    VLOG(0) << "EASL - (MultiThreadedAsyncReader::Consume) split provider returned non-ok status" << status;
...
void MultiThreadedAsyncReader::Add(std::vector<Tensor>& tensors, std::pair<int64_t, string> element_identifier) {
  ...
  VLOG(0) << "blocked_readers " << blocked_readers_ << " and reader_count: " << running_readers_;

  element_index_avail_.emplace(element_identifier);
  // if we do not have the element with the smallest index,
  // we need to wake up all the other readers (which will include the one with smallest element index)
  if (blocked_readers_ == running_readers_ && element_identifier != element_index_avail_.top()) {
    mu_add_cv_.notify_all();
  }
  while (!ProducerSpaceAvailable() 
      || (blocked_readers_ != running_readers_)
...
//    &MultiThreadedAsyncReader::ProducerSpaceAvailable));
  ...
  VLOG(0) << "Add could write to queue with element index " << element_index << " was waiting for " << element_index_;
        ...
        if(s != Status::OK()) {
          ...
          if(!tensors.empty()) {
            ...
            if (deterministic_) {
              ...
              tensors.pop_back();
              VLOG(0) << "right after reading: (" << writer_id << ") with index " << el_indx;
            } else {
  ...
  while(true){
    ...
    if(!deque_.empty()) {
      ...
      if(element.end_of_sequence){
        ...
        VLOG(0) << "num_readers_done_" << num_readers_done_ << "; reader_count_" << reader_count_ << "; running_readers_:" << running_readers_;
        num_readers_done_++;
        deque_.pop_front();
...
//        VLOG(0) << "Got element index " << element.value.back().SummarizeValue(100);
